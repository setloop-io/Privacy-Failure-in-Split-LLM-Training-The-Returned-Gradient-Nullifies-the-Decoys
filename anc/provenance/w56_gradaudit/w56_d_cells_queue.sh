...
run_d() {
  local dim=$1 port=$2
  local cell="gradaudit_d${dim}_10k"
  ...
  log "cell $cell split=14 steps=10000 blocks=256 frames=512 D=$dim port=$port"
  docker run --rm --gpus all --network host --ipc host \
    -e HF_HUB_OFFLINE=1 -e PYTHONUNBUFFERED=1 \
    -e SPLIT_AFTER=14 -e STEPS=10000 -e TRAIN_BLOCKS=256 -e EVAL_BLOCKS=256 \
    -e FRAMES=512 -e LATENT_DIM=$dim -e CELL=$cell -e GRAD_DP=off \
